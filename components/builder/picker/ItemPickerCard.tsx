...
import { Check, Hammer, Coins, Anvil, ShieldAlert, Star, Tag, Clock, ChevronDown } from 'lucide-react';
...
export const ItemPickerCard: React.FC<ItemPickerCardProps> = ({ 
    ...
    const [basisHeight, setBasisHeight] = useState('0px');

    // Calc height on expand
    useEffect(() => {
        if (isExpanded && descRef.current) {
            setBasisHeight(`${descRef.current.scrollHeight}px`);
        } else {
            setBasisHeight('0px');
        }
    }, [isExpanded]);
    ...
    const mainStatsData = MAIN_KEYS.map(key => {
        ...
        return {
            ...
            val: itemVal,
            diff,
            hasStat: itemVal !== 0 || (currentItem && currentVal !== 0),
            isPercent,
    ...
    const handleMouseEnter = () => {
        ...
        hoverTimer.current = setTimeout(() => setIsExpanded(true), 200); // Réduit à 200ms pour plus de réactivité perçue avec l'anim
    };
    ...
    const renderDescription = () => {
        const hasContent = item.description || (item.descriptionBlocks && item.descriptionBlocks.length > 0);
        
        // Si pas de contenu, on met un spacer flex-1 pour pousser le footer en bas
        if (!hasContent) return <div className="flex-1 min-h-[1rem]" />;

        return (
            <div 
                className="relative mt-2 pt-2 border-t border-slate-700/30 flex-1 flex flex-col min-h-[3rem] overflow-hidden transition-[flex-basis] duration-300 ease-out"
                style={{ flexBasis: basisHeight }}
            >
                <div ref={descRef} className="text-[10px] text-slate-400 italic leading-relaxed space-y-1 whitespace-pre-line pb-1">
                    {item.descriptionBlocks && item.descriptionBlocks.length > 0 ? (
                        ...
                        item.description && parseRichText(item.description.replace(/\{\{(.*?)\}\}/g, (_, formula) => {
                            try { return Math.ceil(evaluateFormula(formula, context)).toString(); } catch (e) { return '?'; }
                        }))
                    )}
                </div>
    ...
    return (
        ...
        >
                ...
                <div className="flex items-center min-w-0 pr-2 w-full">
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                            ...
                            {item.isCraftable && <div className="flex items-center justify-center w-6 h-6 rounded bg-emerald-900 border border-emerald-500 text-emerald-400 shadow-sm" title="Craftable"><Anvil size={14} /></div>}
                            {item.isTungsten && <div className="flex items-center justify-center w-6 h-6 rounded bg-red-900 border border-red-500 shadow-sm" title="Tungstène (Unique)"><img src="https://i.servimg.com/u/f19/18/61/88/98/iczne_10.png" alt="Tungstène" className="w-4 h-4 object-contain filter drop-shadow-sm" /></div>}
                            {item.equipmentCost! > 0 && <span className="flex items-center text-[9px] text-slate-300 font-bold bg-slate-800 border border-slate-600 px-2 py-0.5 h-6 rounded"><Hammer size={12} className="mr-1.5 text-slate-400"/>{item.equipmentCost}</span>}
                            {item.goldCost! > 0 && <span className="flex items-center text-[9px] text-yellow-400 font-bold bg-yellow-950/40 border border-yellow-600/50 px-2 py-0.5 h-6 rounded"><Coins size={12} className="mr-1.5 text-yellow-500"/>{item.goldCost}</span>}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className={`flex flex-wrap gap-1.5 mt-2 ${isRestricted ? 'opacity-50' : ''}`}>
                    {otherModifiers.map((m, idx) => {
                        ...
                        if (currentItem) {
                            ...
                            if (Math.abs(diff) > 0) diffDisplay = <span className={diff > 0 ? 'text-emerald-400' : 'text-red-400'}>{diff > 0 ? '▲' : '▼'}</span>;
                        }
                        ...
                        return (
                            <span key={idx} className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-mono font-medium border bg-slate-900/50 border-slate-700 text-slate-300`}>
                                ...
                                {enhanced > 0 ? '+' : ''}{enhanced}{isPercent && '%'} <span className={`mx-1 ${conf.color}`}>{conf.label}</span>
            ...
            {item.tags && item.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-auto pt-2 opacity-60">
                    {item.tags.map(tag => (
