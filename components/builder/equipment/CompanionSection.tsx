...
import { Dog, Zap, User, Plus, Check, X, PawPrint, Shield, Sword, Activity, Heart, AlertCircle, ChevronDown, ChevronUp, Info, FileText } from 'lucide-react';
...
import { toFantasyTitle, AnimatedCounter } from '../../builder/utils';
...
export const CompanionSection: React.FC<CompanionSectionProps> = ({
    ...
    // FIX: Utilisation de finalValue pour capturer TOUS les modificateurs (y compris AltPercent pour Mastodeck)
    const getStatVal = (res: CalculationResult | null, key: string) => {
        if (!res || !res.stats[key]) return 0;
        const rawValue = res.stats[key].finalValue;
        return Math.ceil(rawValue * companionRatio);
    };
    
    const stats = { 
        vit: getStatVal(companionResult, 'vit'), 
        spd: getStatVal(companionResult, 'spd'), 
        dmg: getStatVal(companionResult, 'dmg'), 
        aura: getStatVal(companionResult, 'aura'), 
        res: getStatVal(companionResult, 'res') 
    };
    ...
    const renderStatBox = (label: string, value: number, statKey: string, icon: any, mainColor: string, bgColor: string) => {
        return (
            <div className={`flex flex-col items-center p-3 rounded-lg border relative group ${bgColor} ${mainColor.replace('text-', 'border-').replace('400', '500/20')}`}>
                ...
                <span className="text-xl font-bold text-white tracking-tight"><AnimatedCounter value={value} /></span>
            </div>
        );
    };
    ...
    const renderDetailModal = () => {
        ...
        // FIX: Utilisation de finalValue pour inclure les effets spéciaux (ex: -100% Spd du Mastodeck)
        const rawValue = statRes?.finalValue || 0;
        const finalValue = Math.ceil(rawValue * companionRatio);

        // Valeur partielle pour affichage (peut différer de rawValue si effets spéciaux actifs)
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex flex-col items-center justify-center py-4 rounded-xl border transition-all duration-300 group ${
                                        ...
                                        : 'bg-slate-950 border-slate-800 hover:bg-slate-900 hover:border-slate-700 opacity-60 hover:opacity-100'
                                    }`}
                                >
                                    <Icon 
                                        size={20} 
                        ...
                        <div className="mt-4 bg-slate-950 border border-slate-800 rounded-xl overflow-hidden shadow-lg animate-in fade-in zoom-in-95 duration-300">
                            ...
                            <div className="flex divide-x divide-slate-800 border-t border-slate-800 bg-slate-900/50">
                                <div className="flex-1 py-2 text-center group relative">
                                    ...
                                    <div className="text-sm font-mono text-slate-300">{stats.aura}</div>
                                </div>
                                <div className="flex-1 py-2 text-center group relative">
                                    <div className="text-[9px] text-blue-400 font-bold uppercase mb-0.5">Résistance</div>
                                    <div className="text-sm font-mono text-slate-300">{stats.res}</div>
                                </div>
                            </div>
                            ...
                            {companionDescriptions && companionDescriptions.length > 0 && (
                                <div className="border-t border-slate-800">
                                    ...
                                    {showDetails && (
                                        <div className="px-3 pb-3 space-y-2 max-h-40 overflow-y-auto custom-scrollbar bg-slate-950/50 animate-in fade-in slide-in-from-top-1">
                                            {companionDescriptions.map(desc => (
                                                <div key={desc.id} className="text-[10px] text-slate-400 leading-relaxed pl-3 border-l-2 border-slate-700 py-1">
                                                    <span className="font-bold text-indigo-300 block mb-0.5">{desc.name}</span> 
                                                    <span className="italic opacity-80">{desc.description}</span>
                                                </div>
                                            ))}
                                        </div>
                                    )}
